...
export const MOBS: Record<MobKind, MobDef> = {
  ...
  elon: {
    ...
    attackCooldown: 1.38,
    aggroRange: 15,
    ranged: { cooldown: 2.75, speed: 12.5, damage: 8, color: 0xffffff, size: 0.36, minRange: 4.2, name: 'Tweet' },
    scale: 1.48,
  ...
  helio: {
    ...
    attackCooldown: 1.34,
    aggroRange: 16,
    ranged: { cooldown: 3.15, speed: 11, damage: 8, color: 0x7cff4a, size: 0.34, minRange: 4, name: 'Gosma de taxa' },
    scale: 1.58,
    lines: [
  ...
  boss: {
    ...
    attackCooldown: 1.42,
    aggroRange: 28,
    ranged: { cooldown: 2.7, speed: 11.5, damage: 10, color: 0xff3d5a, size: 0.44, minRange: 4.2, name: 'Corte de Reels' },
    scale: 1.7,
    lines: [
      'Eu AMO pipoca.',
      'Gosto de pipoca.',
...
/** Dano que os inimigos causam. Dificuldade 8 = +8%. */
export function enemyDamageMul(d = DIFFICULTY): number {
  return 1.08 + 0.08 * (clampDifficulty(d) - 8);
}
